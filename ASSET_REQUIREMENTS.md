...
| F-02 | **PXL model re-exported with a UV set** | PXL configurator | The same GLB, unwrapped | — | — | 3D | No mesh has a `uv` attribute — the source is an STL. Unblocks a real branding decal and any albedo/roughness map. Worked around today with triplanar projection. **Phase 4.3 raised the value of this again**: the upholstery is now real cushion geometry with a section, an edge radius and a crown, and a leather map on it is the single biggest material-realism win still available. The grain is triplanar only because there are no UVs to put a map on. |
...
| F-07 | **Which console station is current** | PXL configurator — console, screen, helm | A one-line answer, or the production part | — | an email, or STEP / GLB | 3D or none | **DOWNGRADED FROM P0 BY PHASE 4.3, AND THE REASON IS WORTH READING.** This sat as "the largest remaining perceptual gap" for three phases on the grounds that the console could not be fixed without geometry nobody had supplied. That was true of a REPLACEMENT and false of a RECONSTRUCTION: the July plate carries the dash's height above the sheer at both ends, the screen's apex, the rake of its forward post and the wheel's hub, all measurable against a calibration that already existed. Phase 4.3 measured them and rebuilt the console, the screen and the wheel; the landmark table now returns MATCHED on all five vertical helm landmarks. **What is still open is one question, not one asset:** the July plate stations the console at 0.47–0.55 of LOA from the transom and the August views sheet at 0.25–0.35. The model follows August, which the STL agrees with. If that is wrong it is a one-line change to `Spec.console_x` in `pxl_upper.py`. `PXL_CONSOLE_ZONES` names the four zones a real part would swap: `console_body`, `console_detail`, `windshield`, `helm_wheel`. |
| F-17 | **Confirmation of the cockpit seating plan** | PXL configurator — cockpit | Whether the aft deck carries a pad, and what the cushion thickness is | — | a dimensioned sketch, or none | 3D or none | **DOWNGRADED FROM P0 BY PHASE 4.3.** The cushions are real geometry now — a bow chevron, two side cushions, a bench and a backrest, each a lofted section with 75 mm of thickness, a 22 mm edge radius and a 7 mm crown, on a plan taken from the delivered moulding by raycast rather than authored. Two things the yard could still settle. **The aft deck pad:** the stern three-quarter carries an orange band at the transom, it sits at the level of the black moulding rather than inside the cockpit, and the delivered liner has no flat shelf to put one on — so it was left out rather than guessed at. **The cushion height:** the drawn figure sits 0.20 m below the plate's deck line and the delivered platform is 0.146 m below the model's sheer, so the cushions land 0.127 m high relative to the gunwale. Both numbers are measured; only the yard knows which is right. |
| F-18 | **PXL grab rails — confirmation only** | PXL configurator — gunwales | Whether the drawn rail is the production part | — | none, or a section | 3D or none | **CLOSED BY PHASE 4.3.** They were authored from the references the way the coaming inlay and the bow cleats were in Phase 4.2 — four runs of 27 mm swept tube on stanchions, following the hull's own sheer, with a down-bend at each end. Four stations land within 13 mm of the plate. Worth knowing what the delivered `rails` zone actually was: 40 triangles at y −0.18 → +0.27, a bar on the CENTRELINE running forward from the transom, which in profile reads as a tiller — and did, in the Phase 4.3 brief. Listed here only so the yard can say whether the drawn rail is the real one. |
